...
# The 0.3s time limit aborts the query; it surfaces as TIMEOUT_EXCEEDED, or occasionally as
# QUERY_WAS_CANCELLED when the limit is delivered as a cancellation. Both mean the query failed
# without committing a prefix, so accept either and normalise to the canonical token.
